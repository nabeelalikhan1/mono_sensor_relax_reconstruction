...
function [sig_out,fidexmult] = FAST_IF_ICCD_Sparse_new(Sig,win_length, num, delta,L,thr,Thr,iiii,orderamp)
...
for iii=1:num
    ...
    %figure; plot(IF)
    alpha=0;
    [s1,~,~] = ICCD_sparse(Sig,1,IF,1,orderamp,alpha,iiii);

    Sig(iiii)=Sig(iiii)-s1(iiii);
    %Sig=Sig-s1;
    sig_out=sig_out+s1;
   ...
   % LL=delta;
    ...
    %s2=fftshift(fft(s1));
    %figure; plot(abs(s2));
    if sum(abs(s1.^2))<e_max*thr
        break;
    else
        %  e_max
        if sum(abs(s1.^2))>e_max
            e_max=sum(abs(s1.^2));
        end
...
for i=1:1:orderamp
 [Sig_out1,~,~] = ICCD_sparse(SigO,1,fidexmult,1,i,0,iiii(1:end/2));

 Sig_out1=sum(Sig_out1);
 msee(i)=mean(abs(Sig_out1(iiii(end/2+1:end))-SigO(iiii(end/2+1:end))));
 SS(i,:)=Sig_out1;
end
 
 [~,ind]=min(msee);
[Sig_out1,~,~] = ICCD_sparse(SigO,1,IF,1,3,1,NA);
  sig_out=sum(Sig_out1);

 %sig_out=SS(ind,:);
 
sig_out(iiii)=SigO(iiii);
end
